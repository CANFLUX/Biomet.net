function [numOfFilesProcessed,numOfDataPointsProcessed] = fr_CSI_database(wildCardPath,processProgressListPath,databasePath,time_shift,timeUnit,missingPointValue,chanNames,tableID)
% fr_CSI_database - converts CSI table based data logger files and puts data into
% data base 
% 
% fr_CSI_database(wildCardPath,processProgressListPath,databasePath,time_shift,timeUnit,missingPointValue)
%
% Example:
% [nFiles,nHHours]=fr_CSI_database('d:\Sites\YF\met\YF*.*', ...
%                                  '\\annex001\database\YF\2024\YF_CR1000_progressList.mat','\\annex001\database\yyyy\');
...
%       chanNames               - cell array of all channel names, ie. {'id','xx',...}; 
%       tableID                 - first column of table, should be the single number
% 
%
% Haley Alcock                  File Created:      Aug 14, 2025
%                               Last modification: Aug  14, 2024

% Created based on fr_TOA5_database.m
...
for i=1:length(h)
    ...
    if filesProcessProgressList(j).Modified < datenum(h(i).date)
        try
            ...
            [~,~,tv,outStruct] = fr_read_csi_file(fileName,[],chanNames,tableID);
