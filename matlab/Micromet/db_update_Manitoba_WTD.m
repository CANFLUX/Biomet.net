    ...
    for cntYear=1:length(yearIn)
        for cntSites=1:length(sitesIn)
            ...
            if ~isempty(structIn)
                ...
                fprintf('    %s:%d  Processed: %s\n',siteID,yearIn(cntYear),inputFileName);
            else
                fprintf(2,'    %s:%d  Error while processing: %s\n',siteID,yearIn(cntYear),inputFileName);
            end
...
function outStruct = load_WTD_file(yearIn,inputFileName)
    % The csv files from Manitoba have vtwo types of date formats (see
    % below)
    % This function reads both types automatically
    
    try
        %  Read the datetime column as string (text) first
        ...
        % In case of an error, return an empty structure
        outStruct = struct([]);
    end
end
