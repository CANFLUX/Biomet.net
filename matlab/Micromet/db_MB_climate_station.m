function db_MB_climate_station(fileName,stationName,provinceName,inFileNum)
%
...
%           provinceName: acronym for province where met data is from, e.g. 'MB' (string)
%           inFileNum: ID for file you are using (see list within function)
%                       as they have different formats and are processed
%                       differently.
%
% Revisions:
%
% May 7, 2026 (Rosie)
%   - Renamed function as db_MB_climate_station (from
%     db_Young_climate_station) since it now processes data for other
%     Manitoba (MB) flux sites, e.g. Young, Hogg, and OHM.
%   - Changed dbPath per database meeting (6 May 2026) and Rosie/Zoran discussion
%     (7 May 2026) to group provincial sites by parent "PROVINCIAL" folder then by province 
%     acronym, then 'Met', then station name: % e.g., /yyyy/PROVINCIAL/MB/Met/Newdale/ 
%   - Also changed fullFilePath to match dbPath pattern within Sites folder
%     where raw data files live: e.g., /Sites/PROVINCIAL/MB/Met/.
%
% Apr 28, 2026 (Rosie)
%   - Edited to work with 2025 data file for Newdale (used with Young and Hogg) 
%     and Stonewall (used with OHM) data (inFileNum 6).
%   - 2025 xlsx file is missing an extra data point when the clocks go
%     forward on 9 March, confirmed with Alison Sass (MB gov), no other
%     error related to daylights savings, so handled in same way as other
%     files.
%
% Feb 4, 2025 (Rosie)
...
%   (6) 'Newdale-Stonewall-2025-Hourly-RosieH.xlsx' (all of 2025)
...
elseif strcmpi(fileName,'Newdale-60min-2024.xlsx') | strcmpi(fileName,'Newdale-Stonewall-Hourly2024-AaronG.xlsx')
    ...
    inFileNum = 5;
elseif strcmpi(fileName,'Newdale-Stonewall-2025-Hourly-RosieH.xlsx')
    % File 6: this file has a full year of data for 2025
    inFileNum = 6;
else
...
fullFilePath = fullfile(biomet_sites_default,provinceDir,provinceName,'Met',fileName);

% output path
% following discussion Rosie/Zoran 7 May 2026, we will organize provincial sites by 
% parent "PROVINCIAL" folder then by province acronym, then 'Met', then station name: 
% e.g., /yyyy/PROVINCIAL/MB/Met/Newdale/ 
dbPath = fullfile(biomet_database_default,'yyyy',provinceDir,provinceName,'Met',stationName);   

% old paths (for posterity)
% dbPath = fullfile(biomet_database_default,'yyyy',siteID,'Met',stationName); 
% dbPath = fullfile(biomet_database_default,'yyyy',stationName);     % puts data at same directory level as folder with ECCC data
...
% Per email from Alison (26 Mar 2026): "We found an error in the ET and Solar 
% Radiation calculations in our program so these variables have been removed. 
% The calculations for these variables for previous years is also likely
% invalid." These were calculations of potential solar rad, so measurements
% are still valid.
...
elseif inFileNum == 3
    ...
    % inFileNums 4, 5, and 6: files are each for one full year, 2023 and 2024 respectively
    datetimeTV(idx2).Minute = 0;            % correct "extra" timestamp when daylight savings ends (November)
